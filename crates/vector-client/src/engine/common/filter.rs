...
mod tests {
    ...
    struct TestHandler;
        ...
        fn handle_match(&self, field: &str, value: &str) -> TestCondition {
            TestCondition { field: field.into(), cond_type: format!("match:{}", value) }
        }

        fn handle_match_any(&self, field: &str, values: &[Value]) -> TestCondition {
            TestCondition { field: field.into(), cond_type: format!("match_any:{}", values.len()) }
        }

        fn handle_range(&self, field: &str, _range: &RangeCondition) -> TestCondition {
            TestCondition { field: field.into(), cond_type: "range".into() }
        }

        fn handle_is_empty(&self, field: &str) -> TestCondition {
            TestCondition { field: field.into(), cond_type: "is_empty".into() }
        }

        fn handle_is_null(&self, field: &str) -> TestCondition {
            TestCondition { field: field.into(), cond_type: "is_null".into() }
        }

        fn handle_has_id(&self, ids: &[String]) -> TestCondition {
            TestCondition { field: "_id".into(), cond_type: format!("has_id:{}", ids.len()) }
        }

        fn handle_geo_radius(&self, field: &str, _radius: &GeoRadius) -> TestCondition {
            TestCondition { field: field.into(), cond_type: "geo_radius".into() }
        }

        fn handle_geo_bounding_box(&self, field: &str, _bbox: &GeoBoundingBox) -> TestCondition {
            TestCondition { field: field.into(), cond_type: "geo_bbox".into() }
        }

        fn handle_values_count(&self, field: &str, _count: &ValuesCountCondition) -> TestCondition {
            TestCondition { field: field.into(), cond_type: "values_count".into() }
        }

        fn handle_contains(&self, field: &str, _value: &str) -> TestCondition {
            TestCondition { field: field.into(), cond_type: "contains".into() }
        }

        fn handle_nested(&self, field: &str, filter: TestFilter) -> TestCondition {
            TestCondition { field: field.into(), cond_type: format!("nested:{}_must_cond", filter.must.len()) }
        }
        ...
        fn build_filter(
            ...
            Some(TestFilter { must, must_not, should, min_should })
        }
    }
    ...
    fn test_process_filter_all_types() {
        let filter = VectorFilter::new()
            ...
            .must(FilterCondition::match_any("f2", vec![serde_json::json!("a")]))
            ...
            .must(FilterCondition::geo_radius("f6", GeoRadius::new(GeoPoint::new(1.0, 2.0), 100.0)))
            .must(FilterCondition::values_count("f7", ValuesCountCondition::new().gt(1)))
    ...
    fn test_process_filter_nested() {
        let inner = VectorFilter::new()
            .must(FilterCondition::match_value("inner_field", "val"));
        let nested = FilterCondition {
            field: "nested".into(),
            condition: ConditionType::Nested { filter: Box::new(inner) },
        };
    ...
    fn test_handle_nested_empty_returns_error() {
        let nested = FilterCondition {
            field: "nested".into(),
            condition: ConditionType::Nested { filter: Box::new(VectorFilter::new()) },
        };
        let filter = VectorFilter::new().must(nested);
