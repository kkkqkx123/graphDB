...
async fn test_scroll_all_points() {
    ...
    ctx.insert_test_vectors(1, "Doc", "emb", points).await.expect("insert");

    let collection = "space_1_Doc_emb";
    let (all_points, _next) = ctx.manager.engine().scroll(collection, 100, None, None, None).await.expect("scroll");
...
async fn test_scroll_with_pagination() {
    ...
    let collection = "space_1_Doc_emb";
    let (page1, next) = ctx.manager.engine().scroll(collection, 4, None, None, None).await.expect("scroll");
    assert_eq!(page1.len(), 4);
    assert!(next.is_some(), "should have next page offset");

    let (page2, next2) = ctx.manager.engine().scroll(collection, 4, next.as_deref(), None, None).await.expect("scroll");
    assert_eq!(page2.len(), 4);
    assert!(next2.is_some(), "should have third page offset");

    let (page3, next3) = ctx.manager.engine().scroll(collection, 4, next2.as_deref(), None, None).await.expect("scroll");
...
async fn test_delete_by_filter() {
    ...
    let pts: Vec<VectorPoint> = (0..5).map(|i| {
    ...
    ctx.insert_test_vectors(1, "Doc", "emb", pts).await.expect("insert");

    let filter = VectorFilter::new().must(FilterCondition::match_value("cat", "tech"));
    ctx.manager.engine().delete_by_filter("space_1_Doc_emb", filter).await.expect("delete");
...
async fn test_set_payload() {
    ...
    ctx.manager.engine().set_payload("space_1_Doc_emb", vec!["d1"], payload).await.expect("set payload");

    let point = ctx.get_vector(1, "Doc", "emb", "d1").await.expect("get").expect("exists");
    assert_eq!(point.payload.as_ref().and_then(|p| p.get("status")).and_then(|v| v.as_str()), Some("active"));
}
...
async fn test_delete_payload() {
    ...
    let point = VectorPoint::new("d1", vectors[0].clone())
        .with_payload(create_payload(vec![
            ("keep", serde_json::json!("yes")),
            ("remove", serde_json::json!("bye")),
        ]));
    ctx.insert_test_vectors(1, "Doc", "emb", vec![point]).await.expect("insert");

    ctx.manager.engine().delete_payload("space_1_Doc_emb", vec!["d1"], vec!["remove"]).await.expect("delete payload");

    let point = ctx.get_vector(1, "Doc", "emb", "d1").await.expect("get").expect("exists");
    let p = point.payload.unwrap();
    assert_eq!(p.get("keep").and_then(|v| v.as_str()), Some("yes"));
    assert!(p.get("remove").is_none(), "remove field should be gone");
}
...
async fn test_payload_index_crud() {
    ...
    ctx.manager.engine().create_payload_index("space_1_Doc_emb", "category", PayloadSchemaType::Keyword)
        .await
        .expect("create payload index");

    let indexes = ctx.manager.engine().list_payload_indexes("space_1_Doc_emb").await.expect("list");
    assert!(!indexes.is_empty(), "should have at least one payload index");

    ctx.manager.engine().delete_payload_index("space_1_Doc_emb", "category").await.expect("delete payload index");
}
...
async fn test_collection_info() {
    ...
    let info = ctx.manager.engine().collection_info("space_1_Doc_emb").await.expect("info");
...
async fn test_health_check() {
    let ctx = VectorTestContext::with_dimension(64);
    let status = ctx.manager.engine().health_check().await.expect("health check");
...
async fn test_search_batch() {
    ...
    let queries: Vec<SearchQuery> = vectors[0..3].iter().map(|v| SearchQuery::new(v.clone(), 5)).collect();
    let results = ctx.manager.engine().search_batch("space_1_Doc_emb", queries).await.expect("batch search");
...
async fn test_delete_by_filter_must_not() {
    ...
    let pts: Vec<VectorPoint> = (0..4).map(|i| {
...
async fn test_scroll_nonexistent_collection() {
    let ctx = VectorTestContext::with_dimension(64);
    let result = ctx.manager.engine().scroll("does_not_exist", 10, None, None, None).await;
...
async fn test_set_payload_nonexistent_collection() {
    let ctx = VectorTestContext::with_dimension(64);
    let result = ctx.manager.engine().set_payload("does_not_exist", vec!["x"], Default::default()).await;
...
async fn test_collection_info_nonexistent() {
    let ctx = VectorTestContext::with_dimension(64);
    let result = ctx.manager.engine().collection_info("no_such_collection").await;
    assert!(result.is_err());
}
