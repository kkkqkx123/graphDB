...
async fn test_create_collection_with_hnsw_config() {
    ...
    ctx.manager.create_index("hnsw_test", config.clone()).await.expect("create");

    let meta = ctx.manager.get_index_metadata("hnsw_test").expect("metadata");
...
async fn test_create_collection_with_scalar_quantization() {
    ...
    ctx.manager.create_index("scalar_quant_test", config.clone()).await.expect("create");

    let meta = ctx.manager.get_index_metadata("scalar_quant_test").expect("metadata");
    let stored = &meta.config;
...
async fn test_create_collection_with_product_quantization() {
    ...
    ctx.manager.create_index("product_quant_test", config.clone()).await.expect("create");

    let meta = ctx.manager.get_index_metadata("product_quant_test").expect("metadata");
    let stored = &meta.config;
    let sq = stored.quantization_config.as_ref().expect("quant config");
    let qt = sq.quant_type.as_ref().expect("quant type");
    match qt {
        QuantizationType::Product { compression, always_ram } => {
...
async fn test_create_collection_with_binary_quantization() {
    ...
    ctx.manager.create_index("binary_quant_test", config.clone()).await.expect("create");

    let meta = ctx.manager.get_index_metadata("binary_quant_test").expect("metadata");
    let stored = &meta.config;
    let sq = stored.quantization_config.as_ref().expect("quant config");
    let qt = sq.quant_type.as_ref().expect("quant type");
    match qt {
...
async fn test_create_collection_full_config() {
    ...
    let hnsw = HnswConfig::new(32, 128)
        ...
        .with_on_disk(true);
    let qt = QuantizationType::Scalar { quantile: None, always_ram: Some(true) };
    let qc = QuantizationConfig { enabled: true, quant_type: Some(qt) };
    ...
    ctx.manager.create_index("full_config_test", config.clone()).await.expect("create");

    let meta = ctx.manager.get_index_metadata("full_config_test").expect("metadata");
    let stored = &meta.config;
...
async fn test_create_collection_manhattan() {
    ...
    ctx.manager.create_index("manhattan_test", config).await.expect("create");
