...
async fn test_search_mode_knn() {
    ...
    let search_query = SearchQuery::new(query_vec, 5)
        .with_search_mode(SearchMode::KNN { k: 5, ef_search: Some(64) });

    let results = ctx.manager.search("space_1_Doc_emb", search_query).await.expect("search");
    assert_eq!(results.len(), 5, "KNN should return exactly k results");
    assert!(results[0].score > 0.99, "closest vector should have score near 1.0");
}
...
async fn test_search_mode_range() {
    ...
    let query_vec = vectors[0].clone();
    let search_query = SearchQuery::new(query_vec, 10)
        .with_search_mode(SearchMode::Range { radius: 0.5, max_results: Some(3) });
...
async fn test_search_mode_topk() {
    ...
    let query_vec = vectors[0].clone();
    let search_query = SearchQuery::new(query_vec, 3)
        .with_search_mode(SearchMode::TopK(3));
