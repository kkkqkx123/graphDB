...
async fn test_e2e_geo_radius_filter() {
    ...
    let points = vec![
        VectorPoint::new(1u64, vec![0.1f32; 4]).with_payload({
            ...
            p.insert("location".to_string(), serde_json::json!({"lat": 48.8566, "lon": 2.3522}));
            p
        }),
        VectorPoint::new(2u64, vec![0.2f32; 4]).with_payload({
            let mut p = std::collections::HashMap::new();
            p.insert("location".to_string(), serde_json::json!({"lat": 51.5074, "lon": -0.1278}));
            p
        }),
        VectorPoint::new(3u64, vec![0.3f32; 4]).with_payload({
            let mut p = std::collections::HashMap::new();
            p.insert("location".to_string(), serde_json::json!({"lat": 35.6762, "lon": 139.6503}));
            p
        }),
        VectorPoint::new(4u64, vec![0.4f32; 4]).with_payload({
            let mut p = std::collections::HashMap::new();
            p.insert("location".to_string(), serde_json::json!({"lat": -33.8688, "lon": 151.2093}));
            p
        }),
    ];
    ...
    let results = client.engine().search(&collection, query).await.expect("search");
    assert!(
        results.len() <= 2,
...
async fn test_e2e_geo_bounding_box_filter() {
    ...
    let results = client.engine().search(&collection, query).await.expect("search");
    assert_eq!(results.len(), 2, "Paris and London should be in bounding box");
