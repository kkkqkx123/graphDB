...
use crate::common::{create_empty_index, create_english_index, paginated_search_options};
...
fn test_pagination_total() {
    ...
    for i in 1..=10 {
        index
            .add(
                i,
                &format!("Document number {} about programming", i),
                false,
            )
            .unwrap();
    }
...
fn test_pagination_no_duplicates() {
    ...
    for doc_id in &result1.results {
        assert!(
            !result2.results.contains(doc_id),
            "文档 {} 不应该同时出现在两页中",
            doc_id
        );
    }
