use crate::error::{EncoderError, InversearchError, Result};
...
impl EncoderValidator {
    ...
    pub fn validate(options: &EncoderOptions) -> Result<()> {
        ...
        if let (Some(min), Some(max)) = (options.minlength, options.maxlength) {
            if min > max {
                return Err(InversearchError::Encoder(EncoderError::InvalidRegex(
                    "minlength cannot be greater than maxlength".to_string(),
                )));
            }
            if min == 0 {
                return Err(InversearchError::Encoder(EncoderError::InvalidRegex(
                    "minlength must be greater than 0".to_string(),
                )));
            }
        }
        ...
        if let Some(filter) = &options.filter {
            if filter.len() > 10_000 {
                return Err(InversearchError::Encoder(EncoderError::Encoding(
                    "filter set too large (max 10,000 entries)".to_string(),
                )));
            }
        }
        ...
        if let Some(mapper) = &options.mapper {
            if mapper.len() > 1_000 {
                return Err(InversearchError::Encoder(EncoderError::Encoding(
                    "mapper too large (max 1,000 mappings)".to_string(),
                )));
            }
        }
        ...
        if let Some(matcher) = &options.matcher {
            if matcher.len() > 5_000 {
                return Err(InversearchError::Encoder(EncoderError::Encoding(
                    "matcher too large (max 5,000 patterns)".to_string(),
                )));
            }
        }
        ...
        if let Some(replacer) = &options.replacer {
            if replacer.len() > 100 {
                return Err(InversearchError::Encoder(EncoderError::Encoding(
                    "too many replacer patterns (max 100)".to_string(),
                )));
            }
            ...
            for (pattern, _) in replacer {
                if pattern.len() > 1000 {
                    return Err(InversearchError::Encoder(EncoderError::InvalidRegex(
                        "replacer pattern too long (max 1000 chars)".to_string(),
                    )));
                }
            }
        }
        ...
        if let Some(stemmer) = &options.stemmer {
            if stemmer.len() > 1_000 {
                return Err(InversearchError::Encoder(EncoderError::Encoding(
                    "stemmer too large (max 1,000 rules)".to_string(),
                )));
            }
        }
    ...
    pub fn validate_transformers(
        ...
        if prepare.is_some() && finalize.is_some() {
            ...
            tracing::debug!(
                "Both prepare and finalize transformers are set - this may impact performance"
            );
        }
    ...
    pub fn suggest_optimizations(options: &EncoderOptions) -> Vec<String> {
        ...
        if complexity_score > 50 {
            suggestions
                .push("Consider enabling cache for complex encoder configuration".to_string());
        }
        ...
        if options.numeric != Some(false) {
            suggestions.push(
                "Numeric splitting is enabled - consider disabling if not processing numeric data"
                    .to_string(),
            );
        }
    ...
    fn calculate_complexity_score(options: &EncoderOptions) -> u32 {
        ...
        if options.stemmer.is_some() {
            score += 20;
        }
        if options.matcher.is_some() {
            score += 15;
        }
        if options.mapper.is_some() {
            score += 10;
        }
        if options.replacer.is_some() {
            score += 25;
        }
