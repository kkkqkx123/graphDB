...
impl SymbolTable {
    ...
    /// 更新变量的读取关系（变量重命名）
    pub fn update_read_by(&self, old_var: &str, new_var: &str, node: &PlanNodeRef) -> Result<bool, String> {
        let mut tracker = self.dependency_tracker.write()
            .map_err(|e| format!("Failed to acquire write lock on tracker: {}", e))?;
    ...
    /// 更新变量的写入关系（变量重命名）
    pub fn update_written_by(&self, old_var: &str, new_var: &str, node: &PlanNodeRef) -> Result<bool, String> {
        let mut tracker = self.dependency_tracker.write()
            .map_err(|e| format!("Failed to acquire write lock on tracker: {}", e))?;
    ...
    pub fn get_readers(&self, var_name: &str) -> Result<Vec<PlanNodeRef>, String> {
        ...
        Ok(tracker.find_readers_of(var_name).into_iter().cloned().collect())
    }
    ...
    pub fn get_writers(&self, var_name: &str) -> Result<Vec<PlanNodeRef>, String> {
        ...
        Ok(tracker.find_writers_of(var_name).into_iter().cloned().collect())
    }
    ...
    pub fn detect_write_conflicts(&self) -> Result<Vec<(String, Vec<PlanNodeRef>)>, String> {
        ...
        Ok(tracker.detect_write_conflicts().into_iter()
            .map(|(name, readers)| (name, readers.into_iter().cloned().collect()))
            .collect())
    }

    /// 获取依赖统计信息
    pub fn get_dependency_stats(&self) -> Result<Vec<super::dependency_tracker::DependencyStats>, String> {
        let tracker = self.dependency_tracker.read()
            .map_err(|e| format!("Failed to acquire read lock on tracker: {}", e))?;
    ...
    pub fn allocate_from_pool(&self, key: &str, size: usize) -> Result<Vec<u8>, String> {
        let mut pool = self.obj_pool.write()
