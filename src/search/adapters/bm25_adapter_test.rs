...
mod tests {
    ...
    async fn test_bm25_lifecycle() {
        ...
        let engine = Bm25SearchEngine::open_or_create(temp_dir.path()).expect("Failed to create engine");

        // Test indexing
        engine.index("1", "Rust programming language").await.expect("Failed to index doc 1");
        engine.index("2", "Graph database implementation").await.expect("Failed to index doc 2");
        engine.index("3", "Rust graph database").await.expect("Failed to index doc 3");
        ...
        engine.commit().await.expect("Failed to commit after delete");

        let results = engine.search("Rust", 10).await.expect("Failed to search after delete");
    ...
    async fn test_bm25_batch_operations() {
        ...
        // Batch index
        engine.index_batch(docs).await.expect("Failed to batch index");
        engine.commit().await.expect("Failed to commit");

        // Verify
        let results = engine.search("Document", 100).await.expect("Failed to search");
    ...
    async fn test_bm25_persistence() {
        ...
        // First open, index data
        {
            ...
            engine.index("1", "Persistent data").await.expect("Failed to index");
            engine.commit().await.expect("Failed to commit");
        ...
        // Second open, verify data
        {
            ...
            let results = engine.search("Persistent", 10).await.expect("Failed to search");
            assert_eq!(results.len(), 1, "Expected 1 result");
            assert_eq!(results[0].doc_id, Value::from("1"), "Expected doc_id to be '1'");
        }
    }
    ...
    async fn test_bm25_search_with_limit() {
        ...
        for i in 0..20 {
            engine.index(&i.to_string(), &format!("Common keyword {}", i))
                .await
    ...
    async fn test_bm25_delete_batch() {
        ...
        // Index documents
        for i in 0..10 {
            engine.index(&i.to_string(), &format!("Content {}", i))
                .await
                .expect("Failed to index");
        }
        engine.commit().await.expect("Failed to commit");
        ...
        engine.delete_batch(doc_ids).await.expect("Failed to batch delete");
        engine.commit().await.expect("Failed to commit");

        // Verify
        let results = engine.search("Content", 10).await.expect("Failed to search");
    ...
    async fn test_bm25_stats() {
        ...
        // Index documents
        for i in 0..5 {
            engine.index(&i.to_string(), &format!("Test content {}", i))
                .await
                .expect("Failed to index");
        }
        engine.commit().await.expect("Failed to commit");
    ...
    async fn test_bm25_empty_search() {
        ...
        let results = engine.search("nonexistent", 10).await.expect("Failed to search");
