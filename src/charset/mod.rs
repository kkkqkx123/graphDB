...
impl CharsetUtils {
    ...
    pub fn decode_with_encoding(bytes: &[u8], encoding: Encoding) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        match encoding {
            Encoding::Utf8 => {
            ...
            Encoding::Utf16 => {
                ...
                String::from_utf16(&utf16_vec)
                    .map_err(|_| "Invalid UTF-16 sequence".into())
            },
            Encoding::Latin1 => {
...
impl TextNormalizer {
    ...
    pub fn normalize_diacritics(text: &str) -> String {
        ...
        text.chars()
            .map(|c| match c {
                'á' | 'à' | 'â' | 'ä' | 'ă' | 'ắ' | 'ằ' | 'ẵ' | 'ẳ' | 'ặ' | 'ǟ' | 'ǻ' | 'ǎ' | 'ȁ' | 'ȃ' | 'ȧ' | 'ḁ' | 'ą' | 'ⱥ' | 'ɐ' => 'a',
                'é' | 'è' | 'ê' | 'ë' | 'ĕ' | 'ě' | 'ȅ' | 'ȇ' | 'ȩ' | 'ḝ' | 'ę' | 'ḙ' | 'ḛ' | 'ɇ' | 'ɛ' | 'ǝ' => 'e',
                'í' | 'ì' | 'î' | 'ï' | 'ĭ' | 'ǐ' | 'ȉ' | 'ȋ' | 'į' | 'ḭ' | 'ɨ' | 'ı' => 'i',
                'ó' | 'ò' | 'ô' | 'ö' | 'ő' | 'ŏ' | 'ǒ' | 'ȍ' | 'ȏ' | 'ơ' | 'ǫ' | 'ǭ' | 'ø' | 'ǿ' | 'ɔ' | 'œ' | 'ɶ' | 'ɵ' | 'ȯ' | 'ȱ' | 'ọ' | 'ỏ' | 'ồ' | 'ố' | 'ỗ' | 'ộ' | 'ớ' | 'ờ' | 'ở' | 'ỡ' | 'ợ' => 'o',
                'ú' | 'ù' | 'û' | 'ü' | 'ŭ' | 'ů' | 'ű' | 'ǔ' | 'ȕ' | 'ȗ' | 'ų' | 'ṷ' | 'ų' | 'ʉ' => 'u',
                'ñ' | 'ń' | 'ň' | 'ņ' | 'ŉ' | 'ŋ' | 'ɲ' | 'ŉ' | 'ƞ' | 'ɳ' | 'ȵ' => 'n',
...
mod tests {
    ...
    fn test_encoding_decoding() {
        ...
        let decoded_latin1 = CharsetUtils::decode_with_encoding(&encoded_latin1, Encoding::Latin1).unwrap();
    ...
    fn test_encoding_conversion() {
        ...
        // Convert to Latin-1
        let latin1_bytes = CharsetUtils::convert_encoding(&utf8_bytes, Encoding::Utf8, Encoding::Latin1).unwrap();
