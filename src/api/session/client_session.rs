...
impl ClientSession {
    ...
    pub fn set_space(&self, space: SpaceInfo) {
        *self.space.lock().unwrap() = Some(space);
    }

    pub fn space_name(&self) -> Option<String> {
        self.session.lock().unwrap().space_name.clone()
    }
    ...
    pub fn role_with_space(&self, space: i64) -> Option<RoleType> {
        self.roles.lock().unwrap().get(&space).cloned()
    }
    ...
    pub fn set_role(&self, space: i64, role: RoleType) {
        self.roles.lock().unwrap().insert(space, role);
    }

    pub fn idle_seconds(&self) -> u64 {
        self.idle_start_time.lock().unwrap().elapsed().as_secs()
    }

    pub fn charge(&self) {
        *self.idle_start_time.lock().unwrap() = Instant::now();
    }

    pub fn timezone(&self) -> Option<i32> {
        self.session.lock().unwrap().timezone
    }

    pub fn set_timezone(&self, timezone: i32) {
        self.session.lock().unwrap().timezone = Some(timezone);
    }

    pub fn graph_addr(&self) -> Option<String> {
        self.session.lock().unwrap().graph_addr.clone()
    }

    pub fn update_graph_addr(&self, host_addr: String) {
        self.session.lock().unwrap().graph_addr = Some(host_addr);
    }
    ...
    pub fn update_space_name(&self, space_name: String) {
        self.session.lock().unwrap().space_name = Some(space_name);
    }

    pub fn add_query(&self, ep_id: u32, query_context: String) {
        self.contexts.lock().unwrap().insert(ep_id, query_context);
    }

    pub fn delete_query(&self, ep_id: u32) {
        self.contexts.lock().unwrap().remove(&ep_id);
    }

    pub fn find_query(&self, ep_id: u32) -> bool {
        self.contexts.lock().unwrap().contains_key(&ep_id)
    }
