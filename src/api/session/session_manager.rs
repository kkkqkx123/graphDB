...
use super::client_session::{ClientSession, Session, SpaceInfo, RoleType};
...
impl GraphSessionManager {
    ...
    pub fn find_session(self: &Arc<Self>, session_id: i64) -> Option<Arc<ClientSession>> {
        let sessions = self.sessions.lock().unwrap();
        sessions.get(&session_id).cloned()
    }

    /// Finds an existing session only from local cache
    pub fn find_session_from_cache(self: &Arc<Self>, session_id: i64) -> Option<Arc<ClientSession>> {
        self.find_session(session_id)
    }

    /// Removes a session from local cache
    pub fn remove_session(self: &Arc<Self>, session_id: i64) {
        {
            let mut sessions = self.sessions.lock().unwrap();
            let mut active_sessions = self.active_sessions.lock().unwrap();
...
mod tests {
    ...
    async fn test_session_cache_operations() {
        ...
        let cached_session = session_manager.find_session_from_cache(session.id()).unwrap();
