...
pub(crate) fn get_edge(
    ...
    if let Some(src_label_id) = ctx.graph.get_vertex_label_id(&edge_info.src_tag_name) {
        if let Some(dst_label_id) = ctx.graph.get_vertex_label_id(&edge_info.dst_tag_name) {
