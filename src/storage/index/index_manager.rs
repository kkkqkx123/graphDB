...
impl IndexManager for RedbIndexManager {
    fn update_vertex_indexes(&self, space: &str, vertex_id: &Value, tag_name: &str, props: &[(String, Value)]) -> Result<(), StorageError> {
        ...
        if let Some(space_indexes) = tag_indexes.get(space) {
            for (index_name, index) in space_indexes {
                // 检查索引是否包含此标签的字段
                let should_index = index.fields.iter().any(|f| {
                    props.iter().any(|(prop_name, _)| prop_name == &f.name)
                });
                
                if should_index {
                    // 构建索引键: space:index_name:tag_name:prop_value:vertex_id
                    let mut index_key = format!("{}:idx:v:{}:{}:", space, index_name, tag_name).into_bytes();
    ...
    fn update_edge_indexes(&self, space: &str, src: &Value, dst: &Value, edge_type: &str, props: &[(String, Value)]) -> Result<(), StorageError> {
        ...
        if let Some(space_indexes) = edge_indexes.get(space) {
            for (index_name, index) in space_indexes {
                ...
                if should_index {
                    // 构建索引键: space:index_name:edge_type:prop_value:src:dst
                    let mut index_key = format!("{}:idx:e:{}:{}:", space, index_name, edge_type).into_bytes();
                    for (prop_name, prop_value) in props {
                        if index.fields.iter().any(|f| &f.name == prop_name) {
                            index_key.extend_from_slice(&Self::serialize_value(prop_value));
                            index_key.push(b':');
    ...
    fn delete_vertex_indexes(&self, space: &str, vertex_id: &Value) -> Result<(), StorageError> {
        ...
        if let Some(space_indexes) = tag_indexes.get(space) {
            ...
            for (index_name, _) in space_indexes {
                let prefix = format!("{}:idx:v:{}:", space, index_name).into_bytes();
    ...
    fn build_edge_index_entry(&self, space: &str, index: &Index, edge: &Edge) -> Result<(), StorageError> {
        for field in &index.fields {
            if let Some(value) = edge.props.get(&field.name) {
                // 构建索引键: space:index_name:edge_type:field_name:field_value:src:dst
