...
use crate::storage::index::{RedbIndexPersistence, IndexPersistence};
...
impl IndexData {
    ...
    fn range_lookup_vertex(&self, tag_name: &str, field_name: &str, start_value: &Value, end_value: &Value) -> Vec<Vertex> {
    ...
    fn range_lookup_edge(&self, edge_type: &str, field_name: &str, start_value: &Value, end_value: &Value) -> Vec<Edge> {
    ...
    fn delete_vertex(&mut self, vertex: &Vertex) {
        ...
        for tag in &vertex.tags {
            for (field_name, field_value) in &tag.properties {
                let key = (tag.name.clone(), field_name.clone(), field_value.clone());
    ...
    fn delete_edge(&mut self, edge: &Edge) {
        ...
        for (field_name, field_value) in &edge.props {
            let key = (edge.edge_type.clone(), field_name.clone(), field_value.clone());
...
impl MemoryIndexManager {
    ...
    pub fn add_index(&self, index: Index) -> StorageResult<()> {
        let mut indexes = self.indexes.write().map_err(|e| StorageError::LockError(e.to_string()))?;
        indexes.insert(index.name.clone(), index);
        Ok(())
    }

    pub fn remove_index(&self, name: &str) -> StorageResult<()> {
        let mut indexes = self.indexes.write().map_err(|e| StorageError::LockError(e.to_string()))?;
        indexes.remove(name);
        Ok(())
    }

    pub fn update_index(&self, name: &str, index: Index) -> StorageResult<()> {
        let mut indexes = self.indexes.write().map_err(|e| StorageError::LockError(e.to_string()))?;
    ...
    pub fn get_indexes_by_schema(&self, schema_name: &str) -> Vec<Index> {
        match self.indexes.read() {
            Ok(indexes) => indexes.values().filter(|index| index.schema_name == schema_name).cloned().collect(),
            Err(_) => Vec::new(),
        }
    }
    ...
    pub fn is_field_indexed(&self, schema_name: &str, field_name: &str) -> bool {
        match self.indexes.read() {
            Ok(indexes) => indexes.values().any(|index| {
                index.schema_name == schema_name && index.fields.iter().any(|f| f.name == field_name)
            }),
            Err(_) => false,
        }
    }
...
impl IndexManager for MemoryIndexManager {
    ...
    fn create_index(&self, space_id: i32, mut index: Index) -> StorageResult<i32> {
        let mut indexes = self.indexes.write().map_err(|e| StorageError::LockError(e.to_string()))?;

        if indexes.contains_key(&index.name) {
            return Err(StorageError::AlreadyExists(format!("索引 {} 已存在", index.name)));
        }
    ...
    fn drop_index(&self, _space_id: i32, index_id: i32) -> StorageResult<()> {
        let mut indexes = self.indexes.write().map_err(|e| StorageError::LockError(e.to_string()))?;

        let index_name = indexes.values()
            .find(|idx| idx.id == index_id)
            .map(|idx| idx.name.clone())
            .ok_or_else(|| StorageError::NotFound(format!("索引ID {} 不存在", index_id)))?;
    ...
    fn list_indexes_by_space(&self, space_id: i32) -> StorageResult<Vec<Index>> {
        ...
        Ok(indexes.values().filter(|idx| idx.space_id == space_id).cloned().collect())
    }
    ...
    fn lookup_vertex_by_index(&self, _space_id: i32, index_name: &str, values: &[Value]) -> StorageResult<Vec<Vertex>> {
        let indexes = self.indexes.read().map_err(|e| StorageError::LockError(e.to_string()))?;
        let index = indexes.get(index_name)
            .ok_or_else(|| StorageError::NotFound(format!("索引 {} 不存在", index_name)))?;

        if !matches!(index.status, IndexStatus::Active) {
            return Err(StorageError::IndexError(format!("索引 {} 状态为 {:?}，不可用", index_name, index.status)));
        }

        let index_data = self.index_data.read().map_err(|e| StorageError::LockError(e.to_string()))?;
        let data = index_data.get(&index.id)
            .ok_or_else(|| StorageError::NotFound(format!("索引数据 {} 不存在", index_name)))?;

        let field_name = index.fields.get(0)
            .ok_or_else(|| StorageError::IndexError("索引字段为空".to_string()))?;

        let field_value = values.get(0)
            .ok_or_else(|| StorageError::IndexError("索引值为空".to_string()))?;

        let vertices = data.lookup_vertex_by_property(&index.schema_name, &field_name.name, field_value)
            .ok_or_else(|| StorageError::NotFound("未找到匹配的顶点".to_string()))?;

        Ok(vertices.clone())
    }

    fn lookup_edge_by_index(&self, _space_id: i32, index_name: &str, values: &[Value]) -> StorageResult<Vec<Edge>> {
        let indexes = self.indexes.read().map_err(|e| StorageError::LockError(e.to_string()))?;
        let index = indexes.get(index_name)
            .ok_or_else(|| StorageError::NotFound(format!("索引 {} 不存在", index_name)))?;
        ...
        let edges = data.lookup_edge_by_property(&index.schema_name, &field_name.name, field_value)
            .ok_or_else(|| StorageError::NotFound("未找到匹配的边".to_string()))?;

        Ok(edges.clone())
    }

    fn range_lookup_vertex(&self, _space_id: i32, index_name: &str, start: &Value, end: &Value) -> StorageResult<Vec<Vertex>> {
        let indexes = self.indexes.read().map_err(|e| StorageError::LockError(e.to_string()))?;
        let index = indexes.get(index_name)
            .ok_or_else(|| StorageError::NotFound(format!("索引 {} 不存在", index_name)))?;
        ...
        let vertices = data.range_lookup_vertex(&index.schema_name, &field_name.name, start, end);

        Ok(vertices)
    }

    fn range_lookup_edge(&self, _space_id: i32, index_name: &str, start: &Value, end: &Value) -> StorageResult<Vec<Edge>> {
        let indexes = self.indexes.read().map_err(|e| StorageError::LockError(e.to_string()))?;
        let index = indexes.get(index_name)
            .ok_or_else(|| StorageError::NotFound(format!("索引 {} 不存在", index_name)))?;
        ...
        let edges = data.range_lookup_edge(&index.schema_name, &field_name.name, start, end);

        Ok(edges)
    }

    fn insert_vertex_to_index(&self, _space_id: i32, vertex: &Vertex) -> StorageResult<()> {
        let indexes = self.indexes.read().map_err(|e| StorageError::LockError(e.to_string()))?;
        ...
        for index in indexes.values() {
            ...
            if !matches!(index.index_type, IndexType::TagIndex) {
                continue;
            }

            if let Some(tag) = vertex.tags.iter().find(|t| t.name == index.schema_name) {
                for field in &index.fields {
                    if let Some(field_value) = tag.properties.get(&field.name) {
                        let mut index_data = self.index_data.write().map_err(|e| StorageError::LockError(e.to_string()))?;
                        let data = index_data.entry(index.id).or_insert_with(IndexData::new);
                        data.insert_vertex(&index.schema_name, &field.name, field_value, vertex.clone());
                    }
                }
            }
        }
    ...
    fn delete_vertex_from_index(&self, _space_id: i32, vertex: &Vertex) -> StorageResult<()> {
        let indexes = self.indexes.read().map_err(|e| StorageError::LockError(e.to_string()))?;
    ...
    fn update_vertex_in_index(&self, space_id: i32, old_vertex: &Vertex, new_vertex: &Vertex) -> StorageResult<()> {
        self.delete_vertex_from_index(space_id, old_vertex)?;
        self.insert_vertex_to_index(space_id, new_vertex)?;
        Ok(())
    }

    fn insert_edge_to_index(&self, _space_id: i32, edge: &Edge) -> StorageResult<()> {
        let indexes = self.indexes.read().map_err(|e| StorageError::LockError(e.to_string()))?;
        ...
        for index in indexes.values() {
            ...
            if !matches!(index.index_type, IndexType::EdgeIndex) {
                continue;
            }

            if index.schema_name == edge.edge_type {
                for field in &index.fields {
                    if let Some(field_value) = edge.props.get(&field.name) {
                        let mut index_data = self.index_data.write().map_err(|e| StorageError::LockError(e.to_string()))?;
                        let data = index_data.entry(index.id).or_insert_with(IndexData::new);
                        data.insert_edge(&index.schema_name, &field.name, field_value, edge.clone());
                    }
                }
            }
        }
    ...
    fn delete_edge_from_index(&self, _space_id: i32, edge: &Edge) -> StorageResult<()> {
        let indexes = self.indexes.read().map_err(|e| StorageError::LockError(e.to_string()))?;
    ...
    fn update_edge_in_index(&self, space_id: i32, old_edge: &Edge, new_edge: &Edge) -> StorageResult<()> {
    ...
    fn rebuild_index(&self, space_id: i32, index_id: i32) -> StorageResult<()> {
        ...
        if index.space_id != space_id {
            return Err(StorageError::InvalidInput(format!("索引 {} 不属于空间 {}", index_id, space_id)));
        }

        drop(indexes);

        let storage_engine = self.storage_engine.as_ref()
            .ok_or_else(|| StorageError::StorageError("存储引擎未设置".to_string()))?;
        ...
        match index.index_type {
            IndexType::TagIndex => {
                let vertices = storage_engine.scan_vertices_by_tag("default", &index.schema_name)
            ...
            IndexType::EdgeIndex => {
                let edges = storage_engine.scan_edges_by_type("default", &index.schema_name)
                    .map_err(|e| StorageError::StorageError(e.to_string()))?;
            ...
            IndexType::FulltextIndex => {
                return Err(StorageError::IndexError("全文索引暂不支持重建".to_string()));
            }
        }
    ...
    fn rebuild_all_indexes(&self, space_id: i32) -> StorageResult<()> {
        let indexes = self.indexes.read().map_err(|e| StorageError::LockError(e.to_string()))?;
        let space_indexes: Vec<i32> = indexes.values().filter(|idx| idx.space_id == space_id).map(|idx| idx.id).collect();
        drop(indexes);
    ...
    fn get_index_stats(&self, space_id: i32, index_id: i32) -> StorageResult<IndexStats> {
        ...
        let index_data = self.index_data.read().map_err(|e| StorageError::LockError(e.to_string()))?;
        let data = index_data.get(&index_id)
            .ok_or_else(|| StorageError::NotFound(format!("索引数据 {} 不存在", index_id)))?;
    ...
    fn check_index_consistency(&self, space_id: i32, index_id: i32) -> StorageResult<bool> {
        ...
        match index.index_type {
            IndexType::TagIndex => {
                for (_, vertices) in &data.vertex_by_tag_property {
                    for vertex in vertices {
            ...
            IndexType::EdgeIndex => {
                for (_, edges) in &data.edge_by_type_property {
                    for edge in edges {
    ...
    fn batch_insert_vertices(&self, _space_id: i32, vertices: &[Vertex]) -> StorageResult<()> {
        let indexes = self.indexes.read().map_err(|e| StorageError::LockError(e.to_string()))?;
    ...
    fn batch_delete_vertices(&self, _space_id: i32, vertices: &[Vertex]) -> StorageResult<()> {
        let indexes = self.indexes.read().map_err(|e| StorageError::LockError(e.to_string()))?;
    ...
    fn batch_insert_edges(&self, _space_id: i32, edges: &[Edge]) -> StorageResult<()> {
        let indexes = self.indexes.read().map_err(|e| StorageError::LockError(e.to_string()))?;
    ...
    fn batch_delete_edges(&self, _space_id: i32, edges: &[Edge]) -> StorageResult<()> {
        let indexes = self.indexes.read().map_err(|e| StorageError::LockError(e.to_string()))?;
...
mod tests {
    ...
    fn test_memory_index_manager_creation() {
        ...
        assert!(!manager.has_index("test_index"));
    }
    ...
    fn test_create_and_get_index() {
        ...
        let index_id = manager.create_index(1, index.clone()).unwrap();
        assert_eq!(index_id, 1);
        
        let retrieved = manager.get_index("test_index");
        assert!(retrieved.is_some());
        assert_eq!(retrieved.unwrap().id, index_id);
    }

    #[test]
    fn test_list_indexes_by_space() {
        let temp_dir = TempDir::new().unwrap();
        let manager = MemoryIndexManager::new(temp_dir.path().to_path_buf());
        ...
        let space1_indexes = manager.list_indexes_by_space(1).unwrap();
        assert_eq!(space1_indexes.len(), 1);
        assert_eq!(space1_indexes[0].name, "index1");
    }
}
