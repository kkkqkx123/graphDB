use crate::core::types::{SpaceInfo, TagInfo, EdgeTypeInfo, IndexInfo};
use serde_json;
...
pub fn index_to_bytes(index: &IndexInfo) -> Result<Vec<u8>, StorageError> {
    serde_json::to_vec(index).map_err(|e| StorageError::SerializeError(e.to_string()))
}

pub fn index_from_bytes(bytes: &[u8]) -> Result<IndexInfo, StorageError> {
    serde_json::from_slice(bytes).map_err(|e| StorageError::SerializeError(e.to_string()))
}
