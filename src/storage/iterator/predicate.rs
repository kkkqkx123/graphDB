...
impl SimplePredicate {
    ...
    fn evaluate_value(&self, val: &Value) -> bool {
        match (&self.op, val, &self.value) {
            ...
            (CompareOp::Equal, Value::Float(a), Value::Float(b)) => (a - b).abs() < 1e-6,
            (CompareOp::Equal, Value::Double(a), Value::Double(b)) => (a - b).abs() < 1e-12,
            (CompareOp::Equal, Value::String(a), Value::String(b)) => a == b,
            (CompareOp::NotEqual, Value::Int(a), Value::Int(b)) => a != b,
            (CompareOp::NotEqual, Value::Float(a), Value::Float(b)) => (a - b).abs() >= 1e-6,
            (CompareOp::NotEqual, Value::Double(a), Value::Double(b)) => (a - b).abs() >= 1e-12,
