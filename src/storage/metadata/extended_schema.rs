...
pub struct ExtendedSchemaManager {
...
impl std::fmt::Debug for ExtendedSchemaManager {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ExtendedSchemaManager")
            .field("spaces_count", &self.current_versions.read().len())
            .finish()
    }
}

impl ExtendedSchemaManager {
    pub fn new() -> Self {
        Self {
    ...
    pub fn create_schema_version(&self, space_id: u64) -> Result<i32, ManagerError> {
    ...
    pub fn get_schema_version(&self, space_id: u64) -> Result<i32, ManagerError> {
        let versions = self.current_versions.read();
        Ok(versions.get(&space_id).copied().unwrap_or(0))
    }

    pub fn rollback_schema(&self, space_id: u64, version: i32) -> Result<(), ManagerError> {
        if version < 1 {
    ...
    pub fn get_schema_changes(&self, space_id: u64) -> Result<Vec<SchemaChange>, ManagerError> {
        let changes = self.schema_changes.read();
        Ok(changes.get(&space_id).cloned().unwrap_or_default())
    }

    pub fn clear_schema_changes(&self, space_id: u64) -> Result<(), ManagerError> {
        let mut changes = self.schema_changes.write();
...
impl Default for ExtendedSchemaManager {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn create_test_manager() -> ExtendedSchemaManager {
        ExtendedSchemaManager::new()
    }
