...
use crate::storage::serializer::{
    edge_type_from_bytes, edge_type_to_bytes, space_from_bytes, space_to_bytes, tag_from_bytes,
    tag_to_bytes,
};
...
fn tag_info_to_schema(tag_name: &str, tag_info: &TagInfo) -> Schema {
    let fields: Vec<FieldDef> = tag_info
        .properties
        .iter()
        .map(|prop| FieldDef {
...
fn edge_type_info_to_schema(edge_type_name: &str, edge_info: &EdgeTypeInfo) -> Schema {
    let fields: Vec<FieldDef> = edge_info
        .properties
        .iter()
        .map(|prop| FieldDef {
            name: prop.name.clone(),
...
impl RedbSchemaManager {
    ...
    fn next_tag_id(&self, space: &str) -> Result<i32, StorageError> {
        ...
        let next_id = {
            ...
            let next = current + 1;
            table
                .insert(ByteKey(key.to_vec()), ByteKey(next.to_le_bytes().to_vec()))
                .map_err(|e| StorageError::DbError(e.to_string()))?;
    ...
    fn check_tag_name_conflict(&self, space: &str, tag_name: &str) -> Result<(), StorageError> {
        ...
        if table
            .get(ByteKey(key.as_bytes().to_vec()))
            .map_err(|e| StorageError::DbError(e.to_string()))?
            .is_some()
        {
            return Err(StorageError::DbError(format!(
                "Tag '{}' conflicts with existing edge type in space '{}'",
                tag_name, space
            )));
        }
    ...
    fn check_edge_name_conflict(&self, space: &str, edge_name: &str) -> Result<(), StorageError> {
        ...
        let read_txn = self
            ...
            .map_err(|e| StorageError::DbError(e.to_string()))?;
        let table = read_txn
            .open_table(TAG_NAME_INDEX_TABLE)
            .map_err(|e| StorageError::DbError(e.to_string()))?;
        ...
        {
            return Err(StorageError::DbError(format!(
                "Edge type '{}' conflicts with existing tag in space '{}'",
                edge_name, space
            )));
        }
...
impl crate::storage::metadata::SchemaManager for RedbSchemaManager {
    fn create_space(&self, space: &SpaceInfo) -> Result<bool, StorageError> {
            ...
            .map_err(|e| StorageError::DbError(e.to_string()))?;
        {
            ...
            table
                .insert(ByteKey(key.to_vec()), ByteKey(space_bytes))
                .map_err(|e| StorageError::DbError(e.to_string()))?;
        }
        write_txn
            .commit()
    ...
    fn create_tag(&self, space: &str, tag: &TagInfo) -> Result<bool, StorageError> {
        ...
        {
            // 检查名称索引是否已存在
            let mut name_table = write_txn
                .open_table(TAG_NAME_INDEX_TABLE)
                .map_err(|e| StorageError::DbError(e.to_string()))?;

            if name_table
                .get(ByteKey(name_index_key.as_bytes().to_vec()))
                .map_err(|e| StorageError::DbError(e.to_string()))?
                .is_some()
            {
            ...
            let mut schema_table = write_txn
                .open_table(TAGS_TABLE)
                .map_err(|e| StorageError::DbError(e.to_string()))?;
            schema_table
                .insert(ByteKey(schema_key.as_bytes().to_vec()), ByteKey(tag_bytes))
                .map_err(|e| StorageError::DbError(e.to_string()))?;
        }
        write_txn
            .commit()
    ...
    fn get_tag(&self, space: &str, tag_name: &str) -> Result<Option<TagInfo>, StorageError> {
        ...
        // 2. 从名称索引表获取Tag ID
        let name_table = read_txn
            .open_table(TAG_NAME_INDEX_TABLE)
            .map_err(|e| StorageError::DbError(e.to_string()))?;

        let tag_id = match name_table
            .get(ByteKey(name_index_key.as_bytes().to_vec()))
            .map_err(|e| StorageError::DbError(e.to_string()))?
        {
            Some(value) => {
        ...
        let schema_key = format!("{}:{}", space, tag_id);
        let schema_table = read_txn
            .open_table(TAGS_TABLE)
            .map_err(|e| StorageError::DbError(e.to_string()))?;

        match schema_table
            .get(ByteKey(schema_key.as_bytes().to_vec()))
            .map_err(|e| StorageError::DbError(e.to_string()))?
        {
            Some(value) => {
    ...
    fn list_tags(&self, space: &str) -> Result<Vec<TagInfo>, StorageError> {
            ...
            .map_err(|e| StorageError::DbError(e.to_string()))?
        {
            ...
            if key_str.starts_with(&format!("{}:", space)) {
                ...
                // 获取完整Tag信息
                if let Some(value) = schema_table
                    .get(ByteKey(schema_key.as_bytes().to_vec()))
                    .map_err(|e| StorageError::DbError(e.to_string()))?
                {
    ...
    fn drop_tag(&self, space: &str, tag_name: &str) -> Result<bool, StorageError> {
            ...
            .map_err(|e| StorageError::DbError(e.to_string()))?;
        {
            ...
            // 2. 删除名称索引
            name_table
                .remove(ByteKey(name_index_key.as_bytes().to_vec()))
                .map_err(|e| StorageError::DbError(e.to_string()))?;
            ...
            let mut schema_table = write_txn
                ...
                .map_err(|e| StorageError::DbError(e.to_string()))?;
            schema_table
                .remove(ByteKey(schema_key.as_bytes().to_vec()))
                .map_err(|e| StorageError::DbError(e.to_string()))?;
        }
        write_txn
            .commit()
    ...
    fn create_edge_type(&self, space: &str, edge: &EdgeTypeInfo) -> Result<bool, StorageError> {
            ...
            .map_err(|e| StorageError::DbError(e.to_string()))?;
        {
            ...
            let mut schema_table = write_txn
                ...
                .map_err(|e| StorageError::DbError(e.to_string()))?;
            schema_table
                .insert(ByteKey(schema_key.as_bytes().to_vec()), ByteKey(edge_bytes))
                .map_err(|e| StorageError::DbError(e.to_string()))?;
        }
        write_txn
            .commit()
    ...
    fn get_edge_type(
        ...
        let edge_type_id = match name_table
            .get(ByteKey(name_index_key.as_bytes().to_vec()))
            .map_err(|e| StorageError::DbError(e.to_string()))?
        {
            Some(value) => {
    ...
    fn list_edge_types(&self, space: &str) -> Result<Vec<EdgeTypeInfo>, StorageError> {
            ...
            .map_err(|e| StorageError::DbError(e.to_string()))?
        {
            ...
            if key_str.starts_with(&format!("{}:", space)) {
                let edge_type_id =
                    i32::from_le_bytes(id_bytes.value().0.try_into().unwrap_or([0; 4]));
                let schema_key = format!("{}:{}", space, edge_type_id);
    ...
    fn get_tag_schema(&self, space: &str, tag: &str) -> Result<Schema, StorageError> {
        ...
        let name_index_table = read_txn
            .open_table(TAG_NAME_INDEX_TABLE)
            .map_err(|e| StorageError::DbError(e.to_string()))?;

        let tag_id = match name_index_table
            .get(ByteKey(name_index_key.as_bytes().to_vec()))
            .map_err(|e| StorageError::DbError(e.to_string()))?
        {
            Some(value) => {
            ...
            .map_err(|e| StorageError::DbError(e.to_string()))?
        {
            ...
            None => Err(StorageError::DbError(format!(
                "Tag '{}' schema not found in space '{}'",
                tag, space
            ))),
        }
    }
    ...
    fn get_edge_type_schema(&self, space: &str, edge: &str) -> Result<Schema, StorageError> {
        ...
        let edge_type_id = match name_index_table
            .get(ByteKey(name_index_key.as_bytes().to_vec()))
            .map_err(|e| StorageError::DbError(e.to_string()))?
        {
            Some(value) => {
            ...
            .map_err(|e| StorageError::DbError(e.to_string()))?
        {
            ...
            None => Err(StorageError::DbError(format!(
                "Edge type '{}' schema not found in space '{}'",
                edge, space
            ))),
        }
    }
