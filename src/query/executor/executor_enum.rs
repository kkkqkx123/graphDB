...
pub enum ExecutorEnum<S: StorageClient + Send + 'static> {
    ...
    ShowCreateTag(ShowCreateTagExecutor<S>),
...
impl<S: StorageClient + Send + 'static> Debug for ExecutorEnum<S> {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        let (variant_name, exec_name) = match self {
            ...
            ExecutorEnum::ShowCreateTag(exec) => ("ShowCreateTag", exec.name()),
...
impl<S: StorageClient + Send + 'static> NodeType for ExecutorEnum<S> {
    fn node_type_id(&self) -> &'static str {
        match self {
            ...
            ExecutorEnum::ShowCreateTag(_) => "show_create_tag",
    ...
    fn node_type_name(&self) -> &'static str {
        match self {
            ...
            ExecutorEnum::ShowCreateTag(_) => "Show Create Tag",
    ...
    fn category(&self) -> NodeCategory {
        match self {
            ...
            ExecutorEnum::ShowCreateTag(_) => NodeCategory::Admin,
    ...
    macro_rules! delegate_to_executor {
        ($self:expr, $method:ident) => {
            match $self {
                ...
                ExecutorEnum::ShowCreateTag(exec) => exec.$method(),
