...
mod tests {
    ...
    fn test_filter_vertices_by_labels() {
        ...
        // 按多个标签过滤
        let filtered = matcher.filter_vertices_by_labels(vertices.clone(), &["Person".to_string(), "User".to_string()]);
        assert_eq!(filtered.len(), 2);
        
        // 按不存在的标签过滤
        let filtered = matcher.filter_vertices_by_labels(vertices.clone(), &["NonExistent".to_string()]);
    ...
    fn test_filter_edges_by_types() {
        ...
        // 按多个类型过滤
        let filtered = matcher.filter_edges_by_types(edges.clone(), &["KNOWS".to_string(), "FOLLOWS".to_string()]);
    ...
    async fn test_filter_vertices_by_properties() {
        ...
        let tag1 = Tag::new("Person".to_string(), HashMap::new());
        let vertex1 = Vertex::new_with_properties(
            Value::String("v1".to_string()),
            vec![tag1],
            props1,
        );
        ...
        let vertex2 = Vertex::new_with_properties(
            Value::String("v2".to_string()),
            vec![tag2],
            props2,
        );
        ...
        let filtered = matcher.filter_vertices_by_properties(vertices, &properties, &context).unwrap();
        assert_eq!(filtered.len(), 1);
        assert_eq!(filtered[0].id(), &Value::String("v1".to_string()));
    }
    ...
    async fn test_filter_edges_by_properties() {
        ...
        // 创建属性过滤条件
        let mut properties = HashMap::new();
        properties.insert("weight".to_string(), Expression::Literal(Literal::Float(0.5)));
        
        let filtered = matcher.filter_edges_by_properties(edges, &properties, &context).unwrap();
        assert_eq!(filtered.len(), 1);
