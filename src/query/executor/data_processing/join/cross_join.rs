...
impl<S: StorageEngine> CrossJoinExecutor<S> {
    ...
    fn execute_multi_way_cartesian_product(&self) -> Result<DataSet, QueryError> {
        ...
        for i in 1..self.input_vars.len() {
            ...
            // 执行当前结果与新数据集的笛卡尔积
            current_result = self.execute_two_way_cartesian_product(&current_result, &dataset)?;
        }

        // 设置最终列名
        let mut final_result = current_result;
        final_result.col_names = self.base_executor.get_col_names().clone();
