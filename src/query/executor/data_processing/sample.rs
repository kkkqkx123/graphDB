...
use crate::core::{Value, Vertex, Edge, DataSet};
...
impl<S: StorageEngine> SampleExecutor<S> {
    ...
    /// 执行采样操作
    async fn execute_sample(&mut self, input: ExecutionResult) -> Result<ExecutionResult, QueryError> {
        // 创建评估上下文
        let context = EvalContext::new();
        
        // 评估采样数量
        let count_value = self.evaluator.evaluate(&self.count_expr, &context)
    ...
    fn extract_count(&self, value: &Value) -> Result<usize, QueryError> {
        match value {
            Value::Int(n) => {
                if *n < 0 {
                    return Err(QueryError::ExecutionError("Sample count cannot be negative".to_string()));
                }
                Ok(*n as usize)
            },
            Value::Float(f) => {
            ...
            _ => Err(QueryError::ExecutionError("Sample count must be a number".to_string())),
        }
    }

    /// 值采样
    async fn sample_values(&self, values: Vec<Value>, count: usize) -> Result<Vec<Value>, QueryError> {
        if count >= values.len() {
            return Ok(values);
        }
    ...
    /// 顶点采样
    async fn sample_vertices(&self, vertices: Vec<Vertex>, count: usize) -> Result<Vec<Vertex>, QueryError> {
        ...
        match &self.method {
            ...
            SampleMethod::Stratified(strata_key) => {
                self.stratified_sample_vertices(vertices, count, strata_key).await
            }
        }
    }
    ...
    async fn sample_dataset(&self, dataset: DataSet, count: usize) -> Result<DataSet, QueryError> {
        ...
        let sampled_rows = match &self.method {
            ...
            SampleMethod::Stratified(strata_key) => {
                self.stratified_sample_rows(dataset.rows, &dataset.col_names, count, strata_key).await?
            }
        };
    ...
    /// 分层顶点采样
    async fn stratified_sample_vertices(&self, vertices: Vec<Vertex>, count: usize, strata_key: &str) -> Result<Vec<Vertex>, QueryError> {
        // 按分层键分组
        let mut strata = std::collections::HashMap::new();
        
        for vertex in vertices {
            let stratum_value = self.get_vertex_stratum_value(&vertex, strata_key);
            strata.entry(stratum_value).or_insert_with(Vec::new).push(vertex);
        }
    ...
    /// 分层边采样
    async fn stratified_sample_edges(&self, edges: Vec<Edge>, count: usize, strata_key: &str) -> Result<Vec<Edge>, QueryError> {
        // 按分层键分组
        let mut strata = std::collections::HashMap::new();
        
        for edge in edges {
            let stratum_value = self.get_edge_stratum_value(&edge, strata_key);
            strata.entry(stratum_value).or_insert_with(Vec::new).push(edge);
        }
    ...
    /// 分层行采样
    async fn stratified_sample_rows(&self, rows: Vec<Row>, col_names: &[String], count: usize, strata_key: &str) -> Result<Vec<Row>, QueryError> {
        // 找到分层键的列索引
        ...
        for row in rows {
            ...
            strata.entry(stratum_value).or_insert_with(Vec::new).push(row);
        }
...
impl<S: StorageEngine + Send + 'static> Executor<S> for SampleExecutor<S> {
    async fn execute(&mut self) -> Result<ExecutionResult, QueryError> {
        ...
        // 执行采样操作
        self.execute_sample(input_result).await
    }
...
mod tests {
    ...
    async fn test_sample_executor_random() {
        ...
        impl<S: StorageEngine + Send + 'static> Executor<S> for MockInputExecutor {
            ...
            fn open(&mut self) -> Result<(), QueryError> { Ok(()) }
            fn close(&mut self) -> Result<(), QueryError> { Ok(()) }
            fn id(&self) -> usize { 0 }
