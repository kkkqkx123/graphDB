...
impl<S: StorageEngine + Send + 'static> AppendVerticesExecutor<S> {
    ...
    pub fn with_context(
        ...
        Self {
            base: BaseExecutor::with_context(id, "AppendVerticesExecutor".to_string(), storage, context),
            input_var,
            src_expr,
            props,
            v_filter,
    ...
    fn build_request_dataset(&mut self) -> Result<Vec<Value>, QueryError> {
        ...
        let mut seen = if self.dedup { Some(std::collections::HashMap::new()) } else { None };
...
#[async_trait]
impl<S: StorageEngine + Send + 'static> Executor<S> for AppendVerticesExecutor<S> {
