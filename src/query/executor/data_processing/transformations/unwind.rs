        ...
        match input_result {
            ExecutionResult::Values(values) => {
                ...
                for value in values {
                    ...
                    // 计算展开表达式
                    let unwind_value = self.unwind_expr.evaluate(&expr_context)
            ...
            ExecutionResult::Count(_) => {
                return Err(QueryError::ExecutionError("Cannot unwind count result".to_string()));
            }
        }
...
mod tests {
    ...
    use crate::graph::expression::{Expression, LiteralExpression, VariableExpression};
    ...
    async fn test_unwind_executor() {
        ...
        // 创建UnwindExecutor
        let unwind_expr = Expression::Variable(VariableExpression::new("_".to_string()));
