...
impl MatchValidator {
    ...
    pub fn validate(&mut self) -> Result<(), ValidationError> {
        // 初始化第一个查询部分
        let mut query_parts = vec![QueryPart {
            matchs: Vec::new(),
            boundary: None,
        ...
        validation_context.query_parts = query_parts;
        
        // 执行所有验证策略
        for strategy in &self.validation_strategies {
            if let Err(error) = strategy.validate(&validation_context) {
                self.base.context_mut().add_error(error);
            }
        }
