...
mod tests {
    ...
    fn test_build_columns_for_all_named_aliases() {
        ...
        // 测试空查询部分
        assert!(strategy.build_columns_for_all_named_aliases(&[], &mut columns).is_err());
        
        // 测试有查询部分但无别名
        assert!(strategy.build_columns_for_all_named_aliases(&query_parts, &mut columns).is_ok());
    }
    ...
    fn test_validate_match_clause_context() {
        ...
        assert!(strategy.validate_match_clause_context(&match_context).is_ok());
    }
