...
impl NodePattern {
    pub fn new(identifier: Option<String>, labels: Vec<String>, span: Span) -> Self {
        Self {
            base: BasePattern::new(span, Pattern::Node),
            identifier,
            labels,
            properties: None,
            predicates: Vec::new(),
    ...
    pub fn with_properties(mut self, properties: Expr) -> Self {
        self.properties = Some(properties);
        self
    }
    
    pub fn with_predicates(mut self, predicates: Vec<Expr>) -> Self {
...
impl EdgePattern {
    pub fn new(
        ...
        Self {
            base: BasePattern::new(span, Pattern::Edge),
            identifier,
            edge_type,
            direction,
            properties: None,
...
impl PathPattern {
    pub fn new(elements: Vec<PathElement>, span: Span) -> Self {
        Self {
            base: BasePattern::new(span, Pattern::Path),
            elements,
        }
    }
...
impl VariablePattern {
    pub fn new(name: String, span: Span) -> Self {
        Self {
            base: BasePattern::new(span, Pattern::Variable),
            name,
        }
    }
}
...
impl PatternUtils {
    /// 检查模式是否包含循环
    pub fn has_cycle(pattern: &Pattern) -> bool {
        // 简化的循环检测，实际实现会更复杂
        false
    }
    ...
    /// 检查模式是否有效
    pub fn is_valid(pattern: &Pattern) -> bool {
        // 检查模式的基本有效性
    ...
    /// 合并两个模式
    pub fn merge_patterns(pattern1: &Pattern, pattern2: &Pattern) -> Option<Pattern> {
        // 模式合并逻辑，需要处理变量冲突等问题
        None // 简化实现
    }
}
...
impl PatternFactory {
    /// 创建简单的节点模式
    pub fn node(identifier: Option<String>, labels: Vec<String>, span: Span) -> Pattern {
...
mod tests {
    ...
    fn test_node_pattern() {
        ...
        assert_eq!(node.pattern_type(), Pattern::Node);
    ...
    fn test_edge_pattern() {
        ...
        assert_eq!(edge.pattern_type(), Pattern::Edge);
    ...
    fn test_path_pattern() {
        ...
        assert_eq!(path.pattern_type(), Pattern::Path);
