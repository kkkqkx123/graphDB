...
use super::statement::*;
use super::{
    AstNode, Expression, Pattern, PatternType, Query, Statement, Span,
    VisitorResult,
};
...
pub trait Visitor {
    ...
    // 语句访问方法
    fn visit_statement(&mut self, stmt: &Stmt) -> VisitorResult;
    fn visit_query_statement(&mut self, stmt: &QueryStatement) -> VisitorResult;
    fn visit_create_statement(&mut self, stmt: &CreateStmt) -> VisitorResult;
    fn visit_match_statement(&mut self, stmt: &MatchStmt) -> VisitorResult;
    fn visit_delete_statement(&mut self, stmt: &DeleteStmt) -> VisitorResult;
    fn visit_update_statement(&mut self, stmt: &UpdateStmt) -> VisitorResult;
    ...
    fn visit_use_statement(&mut self, stmt: &UseStmt) -> VisitorResult;
    fn visit_show_statement(&mut self, stmt: &ShowStmt) -> VisitorResult;
    fn visit_explain_statement(&mut self, stmt: &ExplainStmt) -> VisitorResult;
...
impl Visitor for DefaultVisitor {
    ...
    fn visit_match_statement(&mut self, _stmt: &MatchStmt) -> VisitorResult {
        // 简化实现：暂时不访问子句
        Ok(())
    }
    ...
    fn visit_use_statement(&mut self, _stmt: &UseStmt) -> VisitorResult {
        // USE 语句没有子表达式
        Ok(())
    }

    fn visit_show_statement(&mut self, _stmt: &ShowStmt) -> VisitorResult {
        // SHOW 语句没有子表达式
        Ok(())
    }
...
impl Visitor for TypeChecker {
    ...
    fn visit_binary_expr(&mut self, expr: &BinaryExpr) -> VisitorResult {
        // 检查二元表达式的类型兼容性
        match expr.op {
            super::node::BinaryOp::Add
            | super::node::BinaryOp::Sub
            | super::node::BinaryOp::Mul
            | super::node::BinaryOp::Div
            | super::node::BinaryOp::Mod => {
                // 算术操作符需要数值类型
            ...
            super::node::BinaryOp::And | super::node::BinaryOp::Or | super::node::BinaryOp::Xor => {
                // 逻辑操作符需要布尔类型
                if expr.left.expr_type() != super::Expr::Constant
                    || expr.right.expr_type() != super::Expr::Constant
                {
                    self.warnings
                        .push("Logical operations on non-constant expressions".to_string());
                }
            }
            _ => {}
        }
...
impl Visitor for SemanticAnalyzer {
    ...
    fn visit_variable_expr(&mut self, expr: &VariableExpr) -> VisitorResult {
        ...
        if self.lookup_symbol(&expr.name).is_none() {
            self.errors
                .push(format!("Undefined variable: {}", expr.name));
        }
        Ok(())
    }
...
impl Visitor for AstTransformer {
    ...
    fn visit_binary_expr(&mut self, expr: &BinaryExpr) -> VisitorResult {
        // 示例：将 a + 0 转换为 a
        if let (super::node::BinaryOp::Add, Some(constant)) =
            (&expr.op, expr.right.as_any().downcast_ref::<ConstantExpr>())
        {
            if let Value::Int(0) = constant.value {
                self.transformations
                    .push("Optimized a + 0 to a".to_string());
            }
        }
...
mod tests {
    ...
    fn test_type_checker() {
        ...
        let right = Box::new(ConstantExpr::new(
