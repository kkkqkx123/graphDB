...
pub fn build_simple_match_query() -> Stmt {
    ...
    // 创建节点模式: (n:Person)
    let node_pattern = PatternFactory::simple_node(
        Some("n".to_string()),
...
pub fn build_conditional_match_query() -> Stmt {
    ...
    let properties = Expr::Map(MapExpr::new(
        vec![
            ("age".to_string(), Expr::Constant(ConstantExpr::new(Value::Int(25), span))),
        ],
        span,
    ));
...
pub fn build_create_node_query() -> Stmt {
    ...
    let properties = Expr::Map(MapExpr::new(
        vec![
            ("name".to_string(), Expr::Constant(ConstantExpr::new(Value::String("Alice".to_string()), span))),
            ("age".to_string(), Expr::Constant(ConstantExpr::new(Value::Int(30), span))),
        ],
        span,
    ));
...
pub fn build_create_edge_query() -> Stmt {
    ...
    let properties = Expr::Map(MapExpr::new(
        vec![
            ("since".to_string(), Expr::Constant(ConstantExpr::new(Value::Int(2020), span))),
        ],
        span,
    ));
...
pub fn build_complex_pattern() -> Pattern {
    ...
    // 创建节点模式: (a:Person)
    let node_a = PatternFactory::simple_node(
        Some("a".to_string()),
        vec!["Person".to_string()],
        span,
    );
...
pub fn demonstrate_ast_builder() {
    ...
    // 构建简单的 MATCH 查询
    let pattern = PatternFactory::simple_node(
        Some("n".to_string()),
        vec!["Person".to_string()],
        span,
    );
    ...
    // 构建 CREATE 节点查询
    let create_stmt = builder.build_create_node(
        Some("person".to_string()),
        vec!["Person".to_string()],
    );
