...
impl ParserV2 {
    ...
    pub fn parse_go_statement(&mut self) -> Result<Stmt, ParseError> {
        ...
        Ok(Stmt::Go(GoStmt { span, steps, from, over, where_clause, yield_clause }))
    }
    ...
    pub fn parse_lookup_statement(&mut self) -> Result<Stmt, ParseError> {
        ...
        Ok(Stmt::Lookup(LookupStmt { span, target, where_clause, yield_clause }))
    }
    ...
    pub fn parse_subgraph_statement(&mut self) -> Result<Stmt, ParseError> {
        ...
        Ok(Stmt::Subgraph(SubgraphStmt { span, steps, from, over, where_clause, yield_clause }))
    }
    ...
    pub fn parse_find_path_statement(&mut self) -> Result<Stmt, ParseError> {
        ...
        Ok(Stmt::FindPath(FindPathStmt { span, from, to, over, where_clause, shortest, yield_clause }))
    }
    ...
    fn parse_return_clause(&mut self) -> Result<ReturnClause, ParseError> {
        ...
        Ok(ReturnClause { span, items, distinct })
    }
    ...
    fn parse_delete_edge_target(&mut self) -> Result<DeleteTarget, ParseError> {
        ...
        Ok(DeleteTarget::Edges { src, dst, edge_type, rank })
    }
    ...
    fn parse_update_edge_target(&mut self) -> Result<UpdateTarget, ParseError> {
        ...
        Ok(UpdateTarget::Edge { src, dst, edge_type, rank })
    }
    ...
    fn parse_fetch_edges_target(&mut self) -> Result<FetchTarget, ParseError> {
        ...
        Ok(FetchTarget::Edges { src, dst, edge_type, rank, properties })
    }
    ...
    fn parse_create_tag_statement(&mut self, start_span: Span) -> Result<Stmt, ParseError> {
        ...
        Ok(Stmt::Create(CreateStmt {
            span,
            target: CreateTarget::Tag { name: tag_name, properties },
        }))
    }
    ...
    fn parse_create_index_statement(&mut self, start_span: Span) -> Result<Stmt, ParseError> {
        ...
        Ok(Stmt::Create(CreateStmt {
            span,
            target: CreateTarget::Index { name: index_name, on, properties },
        }))
    }
    
    fn parse_create_node_or_edge_statement(&mut self, start_span: Span) -> Result<Stmt, ParseError> {
        // 简化的节点/边创建解析
        // 这里应该解析完整的模式语法
        let span = self.current_span();
