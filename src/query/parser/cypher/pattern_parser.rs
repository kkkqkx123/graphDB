...
impl CypherParserCore {
    ...
    pub fn parse_relationships(&mut self) -> Result<Vec<RelationshipPattern>, String> {
        ...
        self.skip_whitespace();
        while (self.is_current_token_value("-") || self.is_current_token_value("<")) && !self.is_eof() {
...
mod tests {
    ...
    fn test_parse_simple_pattern() {
        let mut parser = CypherParserCore::new("(a:Person)-[:FRIENDS_WITH]->(b:Person)".to_string());
    ...
    fn test_parse_multiple_patterns() {
        let mut parser = CypherParserCore::new("(a:Person)-[:KNOWS]->(b:Person), (c:Company)-[:LOCATED_IN]->(d:City)".to_string());
    ...
    fn test_parse_complex_pattern() {
        let mut parser = CypherParserCore::new("(a:Person)-[:KNOWS*1..2]->(b:Person)<-[:WORKS_WITH]-(c:Company)".to_string());
        let pattern = parser.parse_pattern().unwrap();
