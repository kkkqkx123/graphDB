...
use crate::query::context::ast::{
    AstContext, FetchEdgesContext, FetchVerticesContext, GoContext, LookupContext, MaintainContext,
    PathContext, SubgraphContext,
};
use crate::query::context::ast::{Over, Starts, StepClause};
...
impl QueryParser {
    ...
    pub fn parse_query(ast_ctx: &AstContext) -> Result<Box<dyn QueryContext>, ParseError> {
        ...
        match statement_type.as_str() {
            ...
            stmt if stmt == "SUBMIT JOB"
                || stmt.starts_with("CREATE")
                || stmt.starts_with("DROP") =>
            {
                Ok(Box::new(MaintainContext {
                    base: ast_ctx.clone(),
                }))
            }
    ...
    /// 解析FETCH VERTICES查询
    fn parse_fetch_vertices_query(
        ast_ctx: &AstContext,
    ) -> Result<FetchVerticesContext, ParseError> {
