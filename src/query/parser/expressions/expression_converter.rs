...
/// 将AST表达式转换为graph表达式
pub fn convert_ast_to_graph_expression(
    _ast_expr: &crate::query::parser::ast::Expr,
...
fn convert_arithmetic_op(op: &BinaryOp) -> Result<BinaryOperator, String> {
    match op {
        ...
        BinaryOp::Exp => {
            Err("Exponentiation operator not supported in graph expressions".to_string())
        }
...
fn convert_unary_op(op: &UnaryOp) -> Result<UnaryOperator, String> {
    match op {
        ...
        UnaryOp::IsNotNull => {
            Err("IsNotNull operator not supported in graph expressions".to_string())
        }
        UnaryOp::IsEmpty => Err("IsEmpty operator not supported in graph expressions".to_string()),
        UnaryOp::IsNotEmpty => {
            Err("IsNotEmpty operator not supported in graph expressions".to_string())
        }
    }
}
