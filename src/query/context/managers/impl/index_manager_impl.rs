...
impl IndexManager for MemoryIndexManager {
    ...
    fn lookup_vertex_by_index(
        ...
        if index.status != IndexStatus::Active {
            return Err(ManagerError::IndexError(format!("索引 {} 状态为 {:?}，不可用", index_name, index.status)));
        }
            ...
            if let Some(tag) = vertex.tags.iter().find(|t| t.name == index.schema_name) {
                for field_name in &index.fields {
                    if let Some(field_value) = tag.properties.get(field_name) {
                        ...
                        data.insert_vertex(&index.schema_name, field_name, field_value, vertex.clone());
                    }
                }
            }
        }
...
impl MemoryIndexManager {
    /// 检查顶点是否匹配给定的值
    fn vertex_matches_values(vertex: &Vertex, tag_name: &str, fields: &[String], values: &[Value]) -> bool {
    ...
    /// 检查边是否匹配给定的值
    fn edge_matches_values(edge: &Edge, edge_type: &str, fields: &[String], values: &[Value]) -> bool {
    ...
    fn extract_vertex_field_value(vertex: &Vertex, tag_name: &str, field_name: &str) -> Option<(Value, String)> {
        if let Some(tag) = vertex.tags.iter().find(|t| t.name == tag_name) {
    ...
    fn extract_edge_field_value(edge: &Edge, edge_type: &str, field_name: &str) -> Option<(Value, String)> {
        if edge.edge_type == edge_type {
