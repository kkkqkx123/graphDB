...
impl OptRule for PushFilterDownAggregateRule {
    ...
    fn apply(
        ...
        let agg_child_id = node.dependencies[0];
        let Some(agg_child) = ctx.find_group_node_by_plan_node_id(agg_child_id) else {
            return Ok(None);
        };

        let _agg_node = match &agg_child.plan_node {
            PlanNodeEnum::Aggregate(n) => n,
            _ => return Ok(None),
        };

        if agg_child.dependencies.is_empty() {
            return Ok(None);
        }

        let input_id = agg_child.dependencies[0];

        let input_plan_node = agg_child.plan_node.as_aggregate()
        ...
        let mut new_agg_node = agg_child.plan_node.as_aggregate().unwrap().clone();
        new_agg_node.add_dependency(PlanNodeEnum::Filter(new_filter_node.clone()));

        if let Some(output_var) = agg_child.plan_node.as_aggregate().unwrap().output_var() {
            new_agg_node.set_output_var(output_var.clone());
        }
        new_agg_node.set_col_names(agg_child.plan_node.as_aggregate().unwrap().col_names().to_vec());

        let mut new_agg_group_node = agg_child.clone();
        new_agg_group_node.plan_node = PlanNodeEnum::Aggregate(new_agg_node);
        new_agg_group_node.dependencies = vec![input_id];

        let mut new_filter_group_node = node.clone();
        new_filter_group_node.plan_node = PlanNodeEnum::Filter(new_filter_node);
        new_filter_group_node.dependencies = vec![agg_child_id];

        ctx.add_plan_node_and_group_node(agg_child_id, &new_agg_group_node);

        Ok(Some(new_filter_group_node))
    }
