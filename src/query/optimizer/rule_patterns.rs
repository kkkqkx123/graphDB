...
use super::optimizer::{Pattern, MatchNode};
    ...
    pub fn is_merge_candidate(pattern: &Pattern) -> bool {
        match &pattern.node {
            MatchNode::Single(PlanNodeKind::Filter) => {
                ...
                matches!(&pattern.dependencies[0].node, MatchNode::Single(PlanNodeKind::Filter))
            },
            MatchNode::Single(PlanNodeKind::Project) => {
                // 检查依赖是否也是投影操作
                !pattern.dependencies.is_empty() && 
                matches!(&pattern.dependencies[0].node, MatchNode::Single(PlanNodeKind::Project))
            },
            _ => false,
        }
    }
