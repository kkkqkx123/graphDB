...
use super::visitor::{PlanNodeVisitor, PlanNodeVisitError};
...
macro_rules! impl_plan_node_for {
    ($type:ty) => {
        ...
        impl PlanNodeDependencies for $type {
            fn dependencies(&self) -> &[Arc<dyn PlanNode>] { &self.dependencies }
            fn dependencies_mut(&mut self) -> &mut Vec<Arc<dyn PlanNode>> { &mut self.dependencies }
