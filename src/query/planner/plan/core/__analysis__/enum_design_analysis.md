...
访问者模式是操作 `PlanNodeEnum` 的主要方式，当前存在以下挑战：

1. **Trait 方法数量膨胀**：`PlanNodeVisitor` trait 随着节点类型增加而不断扩展
2. **实现分散**：20+ 处 `impl PlanNodeVisitor` 需要同步维护
3. **默认值缺失**：没有提供默认实现，新方法添加会导致编译错误
...
已实现：
- `plan_node_category.rs`：节点分类系统（8大类）
- `plan_node_cost.rs`：成本模型基础结构（CostEstimate, CostModelConfig, SelectivityEstimate, NodeStatistics）

遇到的问题：
1. **Visitor 实现碎片化**：在尝试实现完整访问者模式时发现，需要为 20+ 个实现添加新方法，工作量巨大且容易遗漏
2. **类型派生约束**：为 `PlanNodeEnum` 添加 `Hash` derive 时，要求所有内部类型也实现 `Eq`，导致编译错误蔓延
3. **模块依赖复杂性**：`plan_node_cost_calc.rs` 和 `plan_node_dependency.rs` 引入的新 trait 需要在多处实现

解决策略：
- 暂缓完整的访问者模式实现，采用现有的基于 `match` 的模式
- 保持 `PlanNodeEnum` 为简单结构（仅 Debug, Clone）
- 成本模型和依赖管理作为独立模块，按需引入

#### 第四阶段建议

基于实施经验，调整第四阶段策略：

1. **增量式实现**：不追求一次性补全所有执行器，而是根据实际查询需求逐步添加
2. **利用现有基础设施**：
   - 使用现有的 `match` 模式处理节点遍历
   - 通过 `plan_node_category.rs` 的分类系统实现批量操作
   - 利用 `plan_node_cost.rs` 的基础结构添加成本计算

3. **优先高频功能**：
   - 优先补全查询语句（SELECT/MATCH）的执行器
   - 管理语句（DDL）可以延后，先保证核心查询功能可用

## 四、下一步工作

### 4.1 短期目标（1-2周）

1. **修复现有警告**：清理 99 个代码警告，提升代码质量
2. **补全缺失的执行器**：根据使用频率选择性地补全
3. **完善 MATCH 规划器**：确保复杂 MATCH 查询能够正确规划

### 4.2 中期目标（1-2月）

1. **优化器规则增强**：补全谓词下推、投影下推等关键规则
2. **成本模型完善**：在现有结构基础上添加成本计算逻辑
3. **管理语句支持**：按需实现 DDL 语句的执行器

### 4.3 长期目标（3-6月）

1. **性能优化**：基于实际运行数据优化成本模型
2. **测试覆盖**：补全单元测试和集成测试
