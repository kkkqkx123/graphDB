...
impl Planner for SubgraphPlanner {
    fn transform(&mut self, ast_ctx: &AstContext) -> Result<SubPlan, PlannerError> {
        ...
        // 1. 创建参数节点，获取起始顶点
        let mut arg_node = Box::new(Argument::new(1, &subgraph_ctx.from.user_defined_var_name));
        ...
        // 5. 如果有标签过滤，添加额外过滤
        let mut tag_filter_node = if let Some(ref tag_condition) = subgraph_ctx.tag_filter {
            let mut filter = Box::new(Filter::new(5, tag_condition));
            filter.set_dependencies(vec![filter_node.clone_plan_node()]);
            filter.set_output_var(Variable {
        ...
        // 6. 如果有边过滤，添加额外过滤
        let mut edge_filter_node = if let Some(ref edge_condition) = subgraph_ctx.edge_filter {
            let mut filter = Box::new(Filter::new(6, edge_condition));
            filter.set_dependencies(vec![tag_filter_node.clone_plan_node()]);
            filter.set_output_var(Variable {
        ...
        project_node.set_dependencies(vec![edge_filter_node.clone_plan_node()]);
