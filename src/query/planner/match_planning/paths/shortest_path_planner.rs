...
impl ShortestPathPlanner {
    ...
    fn find_node_plan(
        ...
        if node_aliases_seen.contains(&node_info.alias) && !node_info.anonymous {
            ...
            return Ok(SubPlan::new(Some(argument_node.clone_plan_node()), Some(argument_node)));
        }
    ...
    fn create_label_index_scan(
        ...
        let plan = SubPlan::new(Some(index_scan_node.clone_plan_node()), Some(index_scan_node));
        Ok(Some(plan))
    }
