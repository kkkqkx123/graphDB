...
impl CypherExpressionOptimizer {
    ...
    pub fn optimize_cypher_expression(cypher_expr: &CypherExpression) -> CypherExpression {
        match cypher_expr {
            ...
            CypherExpression::Binary(bin_expr) => {
                if Self::is_cypher_constant(&bin_expr.left) && Self::is_cypher_constant(&bin_expr.right) {
            ...
            CypherExpression::Unary(unary_expr) => {
                ...
                if Self::is_cypher_constant(&optimized_operand) {
                    Self::try_fold_unary_constant(&unary_expr.operator, &optimized_operand)
                        .unwrap_or_else(|| CypherExpression::Unary(UnaryExpression {
            ...
            CypherExpression::FunctionCall(func_call) => {
                ...
                if optimized_args.iter().all(Self::is_cypher_constant) {
                    Self::try_fold_function_constant(&func_call.function_name, &optimized_args)
                        .unwrap_or_else(|| CypherExpression::FunctionCall(FunctionCall {
            ...
            CypherExpression::Map(map_expr) => {
                ...
                for (key, value) in &map_expr.properties {
                    optimized_properties.insert(key.clone(), Self::optimize_cypher_expression(value));
                }
    ...
    fn try_fold_function_constant(
        ...
        match function_name.to_lowercase().as_str() {
            ...
            "length" if args.len() == 1 => {
                match &args[0] {
                    CypherExpression::Literal(CypherLiteral::String(s)) => {
                        Some(CypherExpression::Literal(CypherLiteral::Integer(s.len() as i64)))
                    }
                    CypherExpression::List(list_expr) => {
                        Some(CypherExpression::Literal(CypherLiteral::Integer(list_expr.elements.len() as i64)))
                    }
                    _ => None,
                }
            }
            "size" if args.len() == 1 => {
                match &args[0] {
                    ...
                    CypherExpression::Map(map_expr) => {
                        Some(CypherExpression::Literal(CypherLiteral::Integer(map_expr.properties.len() as i64)))
                    }
                    _ => None,
                }
            }
    ...
    /// 批量优化Cypher表达式
    pub fn optimize_cypher_batch(
        cypher_exprs: &[CypherExpression],
    ) -> Vec<CypherExpression> {
        cypher_exprs
            .iter()
    ...
    pub fn can_optimize(cypher_expr: &CypherExpression) -> bool {
        match cypher_expr {
            CypherExpression::Binary(bin_expr) => {
                Self::can_optimize(&bin_expr.left) || Self::can_optimize(&bin_expr.right) ||
            ...
            CypherExpression::List(list_expr) => {
                list_expr.elements.iter().any(Self::can_optimize)
            }
            CypherExpression::Map(map_expr) => {
                map_expr.properties.values().any(Self::can_optimize)
            }
            CypherExpression::Case(case_expr) => {
                case_expr.expression.as_ref().map_or(false, |e| Self::can_optimize(e)) ||
                case_expr.alternatives.iter().any(|alt| {
                    Self::can_optimize(&alt.when_expression) || Self::can_optimize(&alt.then_expression)
                }) ||
                case_expr.default_alternative.as_ref().map_or(false, |e| Self::can_optimize(e))
            }
            _ => false,
        }
    }
...
mod tests {
    ...
    fn test_optimize_length_function() {
        let args = vec![CypherExpression::Literal(CypherLiteral::String("hello".to_string()))];
        let expr = CypherExpression::FunctionCall(FunctionCall {
