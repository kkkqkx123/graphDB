...
async fn test_fulltext_phrase_search() {
    ...
    let vertex1 = create_vertex(1, "Article", vec![("content", "The quick brown fox jumps over the lazy dog")]);
    let vertex2 = create_vertex(2, "Article", vec![("content", "Quick thinking leads to quick results")]);
    let vertex3 = create_vertex(3, "Article", vec![("content", "The brown bear in the forest")]);
    ...
    assert_eq!(results[0].doc_id, Value::Int(1), "Best match should be first");
}
...
async fn test_fulltext_boolean_operators() {
    ...
    let vertex1 = create_vertex(1, "Document", vec![("content", "Rust programming language tutorial")]);
    let vertex2 = create_vertex(2, "Document", vec![("content", "Python programming basics")]);
    let vertex3 = create_vertex(3, "Document", vec![("content", "Rust vs Python comparison")]);
...
async fn test_fulltext_large_batch_insert() {
    ...
    for i in 0..BATCH_SIZE {
        let vertex = create_vertex(
            i as i64,
            "Document",
            vec![("content", &format!("Large batch document number {} with some content for testing", i))],
        );
        coordinator
    ...
    assert_eq!(results.len(), BATCH_SIZE, "Should find all {} documents", BATCH_SIZE);
}
...
async fn test_fulltext_multiple_spaces_isolation() {
    ...
    assert_eq!(space1_results[0].doc_id, Value::Int(1), "Should find space 1 document");
    ...
    assert_eq!(space2_results[0].doc_id, Value::Int(2), "Should find space 2 document");
}
...
async fn test_fulltext_cross_space_no_leakage() {
    ...
    let vertex1 = create_vertex(1, "Article", vec![("content", "Unique content for space 1")]);
    ...
    assert_eq!(space2_results.len(), 0, "Should not find space 1 content in space 2");
}
...
async fn test_sync_manager_with_recovery() {
    ...
    sync_manager
        .on_vertex_change(
            ...
            "Document",
            &Value::Int(1),
            &[("content".to_string(), Value::String("Recovery test document".to_string()))],
            ChangeType::Insert,
        )
        .await
...
async fn test_task_buffer_batching() {
    ...
    let buffer = Arc::new(graphdb::sync::batch::TaskBuffer::new(coordinator.clone(), batch_config));
...
async fn test_fulltext_empty_string_content() {
    ...
    let results = coordinator
        .search(1, "Article", "title", "", 10)
        .await;
    
    assert!(results.is_ok() || results.is_err(), "Should handle empty query gracefully");
}
...
async fn test_fulltext_mixed_language_content() {
    ...
    let vertex = create_vertex(
        ...
        vec![(
            "content",
            "Hello 世界 Bonjour Welt こんにちは Hola",
        )],
    );
...
async fn test_fulltext_repeated_same_content() {
    ...
    assert_eq!(results.len(), 5, "Should find all documents with same content");
}
...
async fn test_fulltext_property_type_handling() {
    ...
    let mut props = HashMap::new();
    props.insert("name".to_string(), Value::String("Test Product".to_string()));
    props.insert("price".to_string(), Value::Float(99.99));
    props.insert("quantity".to_string(), Value::Int(100));
    props.insert("description".to_string(), Value::String("A great product".to_string()));
