...
async fn test_fulltext_end_to_end() {
    ...
    // 2. Insert data
    let vertex1 = create_test_vertex(1, "Article", vec![("title", "Hello World"), ("content", "First article")]);
    let vertex2 = create_test_vertex(2, "Article", vec![("title", "Rust Programming"), ("content", "Second article")]);
    let vertex3 = create_test_vertex(3, "Article", vec![("title", "Hello Rust"), ("content", "Third article")]);

    coordinator.on_vertex_inserted(1, &vertex1).await.expect("Failed to insert vertex 1");
    coordinator.on_vertex_inserted(1, &vertex2).await.expect("Failed to insert vertex 2");
    coordinator.on_vertex_inserted(1, &vertex3).await.expect("Failed to insert vertex 3");
    ...
    let results = coordinator.search(1, "Article", "title", "Rust", 10).await.expect("Failed to search");
...
async fn test_fulltext_with_updates() {
    ...
    // Create index
    coordinator.create_index(1, "Post", "content", Some(EngineType::Bm25))
        .await
        .expect("Failed to create index");
    ...
    // Verify
    let results = coordinator.search(1, "Post", "content", "Original", 10).await.expect("Failed to search");
    ...
    coordinator.on_vertex_updated(1, &updated_vertex, &["content".to_string()]).await.expect("Failed to update vertex");
    coordinator.commit_all().await.expect("Failed to commit");
    ...
    let results = coordinator.search(1, "Post", "content", "Updated", 10).await.expect("Failed to search");
...
async fn test_fulltext_rebuild() {
    ...
    // Verify data exists
    let results = coordinator.search(1, "Article", "content", "Test", 10).await.expect("Failed to search");
    assert_eq!(results.len(), 1, "Expected 1 result before rebuild");

    // Rebuild index
    coordinator.rebuild_index(1, "Article", "content").await.expect("Failed to rebuild index");
...
async fn test_fulltext_multiple_fields() {
    ...
    coordinator.create_index(1, "Article", "author", Some(EngineType::Bm25))
        .await
    ...
    // Search each field
    let title_results = coordinator.search(1, "Article", "title", "Rust", 10).await.expect("Failed to search title");
    assert_eq!(title_results.len(), 1, "Expected 1 title result");

    let content_results = coordinator.search(1, "Article", "content", "programming", 10).await.expect("Failed to search content");
    assert_eq!(content_results.len(), 1, "Expected 1 content result");

    let author_results = coordinator.search(1, "Article", "author", "John", 10).await.expect("Failed to search author");
...
async fn test_fulltext_multiple_tags() {
    ...
    coordinator.create_index(1, "Blog", "title", Some(EngineType::Bm25))
        .await
        .expect("Failed to create Blog index");
    coordinator.create_index(1, "News", "title", Some(EngineType::Bm25))
        .await
    ...
    coordinator.on_vertex_inserted(1, &blog_vertex).await.expect("Failed to insert blog");
    coordinator.on_vertex_inserted(1, &news_vertex).await.expect("Failed to insert news");
    coordinator.commit_all().await.expect("Failed to commit");

    // Search each tag
    let blog_results = coordinator.search(1, "Blog", "title", "Blog", 10).await.expect("Failed to search blog");
    assert_eq!(blog_results.len(), 1, "Expected 1 blog result");

    let news_results = coordinator.search(1, "News", "title", "News", 10).await.expect("Failed to search news");
    assert_eq!(news_results.len(), 1, "Expected 1 news result");

    // Cross-tag search should not find results
    let cross_results = coordinator.search(1, "Blog", "title", "News", 10).await.expect("Failed to cross search");
...
async fn test_fulltext_batch_operations() {
    ...
    // Create index
    coordinator.create_index(1, "Document", "content", Some(EngineType::Bm25))
        .await
        .expect("Failed to create index");

    // Batch insert
    for i in 0..100 {
        let vertex = create_test_vertex(i as i64, "Document", vec![("content", &format!("Document number {}", i))]);
        coordinator.on_vertex_inserted(1, &vertex).await.expect("Failed to insert vertex");
    }
    coordinator.commit_all().await.expect("Failed to commit");
    ...
    // Search
    let results = coordinator.search(1, "Document", "content", "Document", 100).await.expect("Failed to search");
...
async fn test_fulltext_delete_and_reinsert() {
    ...
    coordinator.on_vertex_inserted(1, &vertex2).await.expect("Failed to reinsert vertex");
    coordinator.commit_all().await.expect("Failed to commit");

    // Verify re-inserted
    let results = coordinator.search(1, "Article", "title", "Reinserted", 10).await.expect("Failed to search");
...
async fn test_fulltext_empty_and_special_queries() {
    ...
    let results = coordinator.search(1, "Article", "title", "nonexistent", 10).await.expect("Failed to search");
    assert_eq!(results.len(), 0, "Expected 0 results for non-existent term");

    // Search with partial match
    let results = coordinator.search(1, "Article", "title", "Special", 10).await.expect("Failed to search");
...
async fn test_fulltext_concurrent_operations() {
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            for j in 0..10 {
                let vid = i * 10 + j;
                let vertex = create_test_vertex(vid as i64, "Article", vec![("content", &format!("Content {}", vid))]);
    ...
    Arc::try_unwrap(coordinator).expect("Arc still has multiple owners").commit_all().await.expect("Failed to commit");
    tokio::time::sleep(Duration::from_millis(300)).await;
}
