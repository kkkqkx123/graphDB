...
fn test_add_property_rollback_restores_schema() {
    ...
    assert_eq!(result.row_count(), 0, "Property 'age' should not exist after rollback");
...
fn test_delete_property_rollback_restores_property() {
    ...
    assert!(result.row_count() > 0, "Property 'age' should be restored after rollback");
...
fn test_edge_property_rollback() {
    ...
    assert!(result.row_count() > 0, "Edge property 'weight' should be restored after rollback");
