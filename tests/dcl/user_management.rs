...
fn test_create_user_duplicate() {
    ...
    let result = pipeline_manager.execute_query("CREATE USER alice WITH PASSWORD 'password123'");
    assert!(result.is_err(), "Duplicate CREATE USER should fail");
}
...
fn test_change_password_self() {
    // CHANGE PASSWORD without username (self-change) requires active session context
    // Currently this should work as a no-op since CREATE USER provides the context
    new_scenario()
        .exec_dcl("CREATE USER alice WITH PASSWORD 'oldpassword'")
        .assert_success()
        // Self-password-change without username — behavior depends on impl
        // Test that the query at least doesn't panic
...
fn test_dcl_user_name_case_sensitivity() {
    ...
    new_scenario()
        ...
        .assert_success()
        // DESCRIBE with different case — behavior depends on impl
        .exec_dcl("DESCRIBE USER alice")
        .assert_success()
        .exec_dcl("DROP USER Alice")
        .assert_success();
