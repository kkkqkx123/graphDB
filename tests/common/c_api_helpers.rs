...
impl CApiTestSession {
    ...
    pub fn from_db(db: &CApiTestDatabase) -> Self {
        let mut session: *mut graphdb::api::embedded::c_api::types::graphdb_session_t = ptr::null_mut();
...
impl CApiTestTransaction {
    ...
    pub fn commit(self) {
        let rc = unsafe {
            graphdb::api::embedded::c_api::transaction::graphdb_txn_commit(self.txn)
        };
    ...
    pub fn rollback(self) {
        let rc = unsafe {
            graphdb::api::embedded::c_api::transaction::graphdb_txn_rollback(self.txn)
        };
...
impl CApiTestResult {
    ...
    pub fn column_count(&self) -> i32 {
        unsafe {
            graphdb::api::embedded::c_api::result::graphdb_column_count(self.result)
        }
    }
    ...
    pub fn row_count(&self) -> i32 {
        unsafe {
            graphdb::api::embedded::c_api::result::graphdb_row_count(self.result)
        }
    }
}
...
impl CApiTestBatch {
    ...
    pub fn from_session(session: &CApiTestSession, batch_size: i32) -> Self {
        ...
        assert_eq!(rc, graphdb_error_code_t::GRAPHDB_OK as i32, "创建批量插入器失败");
    ...
    pub fn flush(&self) {
        let rc = unsafe {
            graphdb::api::embedded::c_api::batch::graphdb_batch_flush(self.batch)
        };
        assert_eq!(rc, graphdb_error_code_t::GRAPHDB_OK as i32, "执行批量操作失败");
    }
}
