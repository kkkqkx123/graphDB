...
fn test_update_divide_by_zero() {
    ...
    let mut pipeline = QueryPipelineManager::with_optimizer(storage.clone(), stats_manager, optimizer)
        .with_schema_manager(schema_manager);

    let space_name = "test_space";
    let create_space = format!("CREATE SPACE IF NOT EXISTS {}", space_name);
    pipeline.execute_query(&create_space).expect("Failed to create space");

    let space_info = {
        let storage_guard = storage.read();
        storage_guard.get_space(space_name).expect("Failed to get space").expect("Space not found")
    };

    let create_tag = "CREATE TAG Counter(val INT)";
    pipeline.execute_query_with_space(create_tag, Some(space_info.clone())).expect("Failed to create tag");

    let insert = "INSERT VERTEX Counter(val) VALUES 1:(10)";
    pipeline.execute_query_with_space(insert, Some(space_info.clone())).expect("Failed to insert vertex");

    let update = "UPDATE 1 SET val = val / 0";
    let result = pipeline.execute_query_with_space(update, Some(space_info.clone()));
    assert!(result.is_err(), "UPDATE with division by zero should return an error, got {:?}", result);
}
