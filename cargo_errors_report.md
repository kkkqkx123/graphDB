...
- Line 307: unused import: `tokio_test`
- ... 3 more occurrences in this file
...
- Line 8: unused import: `FulltextQueryExpr`
...
- Line 45: manual implementation of `ok`: help: replace with: `receiver.try_recv().ok()`
...
- Line 126: casting to the same type is unnecessary (`f64` -> `f64`): help: try: `context.score`
- Line 189: this `if let` can be collapsed into the outer `if let`
- Line 263: this `if let` can be collapsed into the outer `if let`
...
- Line 106: manual `!RangeInclusive::contains` implementation: help: use: `!(0.0..=1.0).contains(&b)`
...
- Line 165: called `map(..).flatten()` on `Iterator`: help: try replacing `map` with `flat_map` and remove the `.flatten()`: `flat_map(|v| v.values())`
