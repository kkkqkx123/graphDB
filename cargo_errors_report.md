...
- Line 1078: unused variable: `regex_pattern`: help: if this is intentional, prefix it with an underscore: `_regex_pattern`
...
- Line 187: field `edge_type` is never read
- Line 244: fields `vertex_ids`, `edge_direction`, and `edge_types` are never read
- Line 313: fields `vertex_ids`, `edge_ids`, and `prop_names` are never read
...
- Line 1179: fields `storage`, `validate_context`, `inputs`, `space`, and `vid_type` are never read
- Line 1199: multiple methods are never used
...
- Line 530: methods `execute_merge_sort`, `merge_sort`, and `merge_impl` are never used
