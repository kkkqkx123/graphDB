...
**语法结构**:
```cypher
FIND <SHORTEST|ALL> PATH FROM <src_id> TO <dst_id> OVER <edge_type> [REVERSELY] [BIDIRECT] [WHERE <condition>] [UPTO <steps> STEPS] [WEIGHT <weight_expr>]
```
...
**注意**: NOLOOP选项已作为默认行为实现，因此无需显式指定。所有路径查找算法默认都会避免循环路径。
...
**语法结构**:
```cypher
DELETE VERTEX <vertex_id> [, <vertex_id> ...]
